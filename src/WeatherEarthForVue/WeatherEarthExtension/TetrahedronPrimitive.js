...
function createFragmentShader() {
  const fragmentShader = `
    ...
    void main()
    {
        ...
        material.diffuse = (1.0 - colorImage.a) * vec3(1.0,0.0,0.0) + colorImage.rgb * vec3(1.0 - vst, 1.0, vst);
     #ifdef FLAT
...
function createTexture(tetrahedronPrimitive) {
  Cesium.Resource.createIfNeeded(tetrahedronPrimitive._imageUrl).fetchImage().then((image) => {
    let vTexture;
...
function computeHeight(tetrahedronPrimitive) {
  const point = Cesium.Cartesian3.fromElements(0, 0, tetrahedronPrimitive._distance, new Cesium.Cartesian3());
  const enuPoint = Cesium.Matrix4.multiplyByPoint(tetrahedronPrimitive._enuMatrix, point, new Cesium.Cartesian3());
  const upPositionEC = Cesium.Matrix4.multiplyByPoint(tetrahedronPrimitive._viewer.scene.camera._viewMatrix, enuPoint, new Cesium.Cartesian3());
  const upPositionPC = Cesium.Matrix4.multiplyByPoint(tetrahedronPrimitive._viewer.scene.camera.frustum.projectionMatrix, upPositionEC, new Cesium.Cartesian3());
...
function TetrahedronPrimitive(options) {
  ...
  this._imageUrl = require('@/assets/images/fence.png');
...
TetrahedronPrimitive.prototype.startAnimate = function () {
  ...
  function animateFunc() {
    ...
    const rotationZ = Cesium.Matrix4.fromRotationTranslation(Cesium.Matrix3.fromRotationZ(Cesium.Math.toRadians(that._speed)));
